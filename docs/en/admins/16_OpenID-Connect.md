...
* `OIDC_STATE_TIMEOUT`: Optional. Lifetime in seconds of the transient state/nonce (anti-CSRF) cookie that is set while an authentication request is in flight. When not defined, the default is 300 seconds — the same as mod_auth_openidc — which keeps the anti-CSRF window tight; it is exposed only so operators can raise it if a particular IdP flow needs longer. Note that a login which outlives this window no longer fails hard when `OIDC_DEFAULT_URL` is set (see below). See [mod_auth_openidc’s documentation for details](https://github.com/OpenIDC/mod_auth_openidc/blob/72c9f479c2d228477ff0a9518964f61879c83fb6/auth_openidc.conf#L802-L804).
* `OIDC_DEFAULT_URL`: Optional. URL the browser is redirected to when there is no valid session and no explicit target — **including when an authorization response carries an expired or unmatchable state** (for example an interactive login that took longer than `OIDC_STATE_TIMEOUT`). Without it, mod_auth_openidc returns a hard `Bad Request` (`oidc_response_proto_state_restore: state has expired`); with it, the module simply restarts the login and the user ends up signed in. When not defined, the default is `/i/` (the FreshRSS index). See [mod_auth_openidc’s documentation for details](https://github.com/OpenIDC/mod_auth_openidc/blob/72c9f479c2d228477ff0a9518964f61879c83fb6/auth_openidc.conf#L60-L64).
